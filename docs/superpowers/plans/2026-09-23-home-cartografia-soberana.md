...
| `src/app/globals.css` | T1 (`@source not`), T5, T6, T7 (bloque «Tesis y Capacidades»; borra `.bg-grid`), T8 (bloque «Del papel al dato»: estado final sin JavaScript y variantes locales `dossier-compact` / `dossier-narrow`), T10 |
...
| `src/components/providers/SmoothScroll.tsx` | T5, T8 (revisión: anclas nativas y URL con ancla, decisión 28) |
...
nav: {
  items: [
    ...
    { name: "Productos", opensMenu: true, menuBlock: "products" },  // en: "Products"; menuBlock: añadido en la revisión de T5 (decisión 13)
...
| `data-capability-edge` | cada arista (`m.line`) de la red de Capacidades (T7, F3) | — |
...
| `data-dossier-status` | cada una de las tres etiquetas de estado apiladas en una fila del panel (solo se ve la del estado de la fila; T8, ronda 3 de la revisión) | `idle` \| `active` \| `done` |
...
| 3 | Capacidades | `capacidades` / `capacidades` | claro | `home/CapabilitiesIndex.tsx` (+ `CapabilitiesMedia.tsx`, imagen de la lámina como slot) |
| 4 | Del papel al dato | `gobierno` / `government` | oscuro | `home/Dossier.tsx` (+ `DossierStage.tsx`, la isla, y `dossier-progress.ts`) |
...
- El marco `sticky` (`h-svh` + `overflow-hidden`) nunca recorta contenido ni lo deja bajo el encabezado fijo de 64 px: `scrub` empieza en 600 px de alto, así que una sección cuyo contenido pueda no caber compacta su ritmo con `short:` (≤ 720 px de alto) y lleva además `stage:h-auto stage:min-h-svh` en `frameClassName` para crecer en lugar de recortar (portada en T6; el expediente de T8 compacta con dos variantes locales de `globals.css`, siempre junto a `stage:`: `dossier-compact:`, hasta 48rem de alto y, en la franja md estrecha de hasta 56rem de ancho, hasta 54rem; y `dossier-narrow:`, que en esa franja compacta además el título y el panel. Decisión 8 de T8).
...
| 7 | Tesis y Capacidades | `e2e/home.spec.ts` | `home/{Thesis,CapabilitiesIndex,CapabilitiesMedia}.tsx` | `Services.tsx`, `FlagshipAI.tsx`, clave `flagshipAI`, `.bg-grid` |
| 8 | Del papel al dato | `e2e/home.spec.ts` | `home/{Dossier,DossierStage}.tsx`, `home/dossier-progress.ts` | — |
...
- **Sin cambios:** `src/components/layout/header/HamburgerIcon.tsx`, `src/components/ui/LanguageSwitcher.tsx` y `src/components/ui/Button.tsx`
- **Modificado en la revisión:** `src/app/[locale]/layout.tsx`, solo `data-scroll-behavior="smooth"` en `<html>` (decisión 13 i); sigue pasando `dict.nav` y `dict.footer`
- **Borrar:** —
...
```
Después: nada (el archivo sigue con `function serviceLines…`). En la ronda 1 de la revisión `serviceLines` perdió además el parámetro `dict`, que no usaba, y con él el import de `Dictionary`: la firma integrada es `serviceLines(locale, links)` y las llamadas de (c) quedan como `serviceLines("es", es.nav.platformLinks)` (decisión 13 h).
...
13. **Lo que el código de arriba no recoge (implementación y rondas 1 a 3 de la revisión).** Manda el código integrado, con `e2e/header.spec.ts`. (a) Contrato §3.2, cambio aditivo: `nav.items[1]` gana `menuBlock: "products"` en `es.ts` y `en.ts`, y `NavItem` gana el campo opcional `menuBlock: MenuBlockId`. "Productos" abre el menú con el foco en RealTy (`[data-menu-block="products"]`); "Servicios" y la hamburguesa, en el primer enlace del panel. La prueba de foco ya no usa `menu.getByRole('link').first()` para los dos botones. (b) Con el menú abierto, `<main>` y `.site-footer` llevan `inert`, y el anillo de foco es blanco en `#site-mega-menu` y `.site-footer`; la prueba comprueba estilo (`solid`), grosor (`2px`) y color del contorno, no solo el color. (c) Cualquier cambio de ruta cierra el menú: estado derivado de `pathname` en el render, que cubre "Empresa" y Atrás/Adelante. "Empresa" cierra además al pulsarse, como el logo y el CTA (decisión 8). (d) `BrandLogo` lleva `tabIndex={-1}`: el `whileTap` de motion añadía una parada de tabulación sin nombre dentro del enlace del logo. (e) Por debajo de 360 px, el grupo de acciones usa `gap-2` y el CTA `px-2.5`: a 320 px no toca el logo. (f) El footer lleva `data-header-theme="dark"`, 7 columnas desde `xl` y el encabezado sin JavaScript pasa a barra oscura al bajar (animación ligada al scroll). (g) `SmoothScroll` (viene de la implementación inicial, no de la revisión): salto instantáneo solo al cambiar de ruta; en el montaje se mantiene el `scrollTo(0, 0)` suave de siempre, porque un salto instantáneo al hidratar anulaba el scroll y el clic que el visitante hiciera antes (fallaba `copy.spec`). Compara el `pathname` anterior guardado en un ref, no una marca de "primera ejecución", para que el doble montaje de StrictMode siga en la rama de montaje. (h) Código muerto quitado: el parámetro `dict` de `serviceLines` y el import de `Dictionary` en `llms.txt/route.ts` (firma `serviceLines(locale, links)`), `getMegaMenuLabels().company` y `NavContent.contact`. La clave `nav.contact` de los diccionarios se queda para T11 (§3.2 "sin cambios"). (i) `<html>` lleva `data-scroll-behavior="smooth"` en `src/app/[locale]/layout.tsx`: Next 16 solo desactiva el scroll suave durante su propio scroll de navegación si está el atributo, y sin él avisa en desarrollo en cada navegación de cliente. La rama instantánea de `SmoothScroll` se queda, porque el atributo solo cubre el scroll de Next y el de `SmoothScroll` corre después, en un efecto. (j) Dos `<nav>` con nombre: la del encabezado ("Principal" / "Main") y la del panel ("Menú del sitio" / "Site menu"), que envuelve la rejilla de bloques. Por debajo de `md` la nav del encabezado no se muestra y la del panel es la única región de navegación; en escritorio, con el menú abierto, conviven las dos con nombres distintos. Los nombres salen de `getNavLandmarkLabels` en `header/types.ts`, no de los diccionarios, así que §3.2 no cambia.
...
> **Borrador superado.** El código integrado se aparta de este borrador (y del de 2.4) en lo que recoge «Decisiones (implementación, F3)» al final de esta tarea: la imagen llega como slot `media` desde `CapabilitiesMedia.tsx`, el avance sale de `useStageProgress` sobre la sección, la lámina es `w-full`, la lista lleva un relleno inferior en `stage:`, la secuencia de celular va con `once` y hay CSS propio en `globals.css`. Manda el código; no copies este bloque.
...
### Decisiones (implementación, F3)
Lo que el código de `wt/task7` cambia respecto de los borradores de 2.3/2.4 y del paso 1 (commit de la tarea y ronda 1 de revisión). Manda el código.

11. **Imagen de la lámina como slot** (presupuesto de JS, informe §5, T7). `src/components/sections/home/CapabilitiesMedia.tsx` (sin directiva) renderiza `<Image>` en el servidor con `coverPlaceholder`, carga perezosa y `sizes` en rem ajustado al marco real (ver la decisión 22). `page.tsx` lo pasa a `CapabilitiesIndex` como prop `media`: ni `next/image` ni la vista previa de la lámina viajan en el JS de la isla.
12. **`useStageProgress` sobre la sección** (offset `["start end", "end start"]`) en lugar de `useScroll()` global: da el modo y avisa de cada scroll con la sección en pantalla, que es cuando el índice puede cambiar. Las filas se buscan una vez al entrar en scrub y se guardan en un ref; `indexAtCenter` deja de leer en la primera fila que aún no llegó al centro.
13. **Relleno inferior de la lista en `stage:`.** Sin él, cuando la red se completa (borde inferior de la última fila en el centro) la columna de la lámina ya terminó y la lámina está a medio salir por arriba (323 px fuera a 1440×900). El relleno es lo que el borde inferior de la figura fija pasa del centro, nunca negativo: `max(0, min(50svh, 5/9 · min(100vw, 80rem) + 5rem − 50svh))`. Sale de la columna de 5/12 (`5/12 · min(100vw, 80rem) − 3rem`), la lámina de alto `min(4/3 · columna, 100svh − 8rem)`, el `top-24` y la leyenda en una o dos líneas. *Ronda 1:* la primera versión usaba un término fijo de 48,5rem (la columna a su ancho máximo) y dejaba 137–264 px en blanco de sobra entre 768 y 1024 px de ancho; ahora sobran ~20 px con la leyenda en una línea (1440×900: 341 px frente a 322 necesarios; 820×1180 y 768×1024: 0).
14. **Lámina `w-full`** (*ronda 1, hallazgo mayor*). Con ancho automático y `aspect-ratio`, el `md:max-h-[calc(100svh-8rem)]` también estrechaba la caja (el tope pasa al ancho por la proporción): a 1366×657 la lámina medía 397 px en una columna de 485 y el contador «0N / 08» quedaba 89 px a la derecha de la imagen (107 px a 844×390). Con `w-full`, en pantallas bajas la lámina es más baja, no más estrecha; `FocalCover` recorta y los 8 nodos (x 490–1060) siguen dentro. Es lo que describía el traspaso («siempre deja ver al menos x 384–1152»).
15. **Leyenda:** el contador va con `shrink-0 whitespace-nowrap` (a 768 px se partía en «08 /» y «08») y `aria-hidden` (es un estado visual del scroll; la leyenda para lectores de pantalla es solo «Imagen ilustrativa»); la etiqueta lleva `min-w-0` y es la única que puede partirse (a 768 px, en dos líneas: el relleno de la decisión 13 lo cubre). Los índices `/0N` usan `InstrumentLabel` (con `text-[#0a0a0a]` encendidos), como pide el contrato para la capa de instrumento.
16. **Celular: la secuencia va con `once`** (*ronda 1*). Sin `once`, al salir la lámina de pantalla el intervalo se cancelaba y la red se quedaba a medias (p. ej. «03 / 08» con un nodo azul fuera de la vista) hasta volver a subir. Con `once`, como la tesis y el diseño §5 («una vez»), una vez disparada termina siempre en N. Efecto aceptado: al pasar de escritorio a celular con la lámina ya vista en scrub, la secuencia arranca sin esperar a verla de nuevo, y termina igualmente en N.
17. **Sin JavaScript se pinta el estado final** (*ronda 1*), como la portada de T6 y como con reducir movimiento: bloque `@media (scripting: none)` en `globals.css` que pone la tesis y los índices `/0N` en `#0a0a0a`, rellena los nodos, quita el guion de las aristas (`data-capability-edge`, atributo nuevo en §3.4) y oculta el contador («00 / 08» no significaba nada). Nada azul. Sustituye a la decisión 6 en lo que dice de la tesis sin JavaScript.
18. **Aristas y nodos en pantallas bajas** (*rondas 1 y 2*). Con la lámina de ~262 px de alto (844×390), 2,5 unidades del viewBox eran 0,64 px. `@media (height < 37.5rem)` sube el grosor a 4 unidades (≈ 1 px). En tabletas verticales en scrub (768–820 px de ancho) quedan en 0,88–0,95 px con 2,5 unidades: se acepta, ≈ 1 px (§3.6). *Ronda 2:* en el mismo bloque, cada `[data-node]` se escala ×1,5 sobre su centro (`transform-box: fill-box`): a 844×390 el nodo pasa de 6,1 a 9,1 px y el anillo del activo de 12 a 18 px, como en un celular vertical (10 y 20 px). Escala y no `r` en CSS, que no todos los navegadores aceptan como propiedad; los trazos de los círculos son `non-scaling-stroke` y no engordan, y los nodos siguen en los extremos de sus aristas.
19. **`.bg-grid` sale de `globals.css`:** solo lo usaba `Services.tsx`, que esta tarea borra. El skill `orbexs-design-system` (`tokens.md`, `layout.md`) aún lo cita como patrón de Services: lo actualiza T11 con la referencia de movimiento.
20. **Pruebas añadidas en la ronda 1** (spec temporal `e2e/home-t7.spec.ts`, que el integrador funde en `e2e/home.spec.ts`): estado final sin JavaScript; aristas enteras y nodos negros con reducir movimiento; tesis a mitad de recorrido (las activas son un prefijo con 0 < k < total); final del scrub a 1440×900, 1366×657, 1024×768, 768×600 y 768×1024 (08 / 08, sin azul, lámina fija en `top-24`, entera, del ancho de su columna y con el contador alineado y en una línea); celular 390×844 (tesis que se revela entera y secuencia que termina en N aunque la lámina salga de pantalla). La prueba «revisión 3» espera la hidratación y enciende la fila 4 en scrub antes de pasar a celular.
21. **320 px** (*ronda 2*). La matriz de viewports del traspaso (§8) no incluye 320 px, pero a 320×568 (es) «organizaciones» y «ciberseguridad» (tesis a 2,5rem, el mínimo de `text-fluid-h1`, en una columna de 272 px) sobresalían 8–13 px del párrafo y se comían el margen derecho, y «Automatización» y «Enriquecimiento» (`text-2xl` en una columna de título de 174 px) se salían de su h3. Por debajo de 22,5rem (`max-[22.5rem]:`, en rem como §3.6) la tesis baja a 2,25rem y los títulos a `text-xl`: todo cabe en es y en en, y desde 360 px no cambia nada. Se descartó `hyphens: auto`: depende de los diccionarios de guionado de cada navegador y partiría palabras de una frase que se lee de un vistazo.
22. **`sizes` de la lámina exacto** (*ronda 2*). El valor anterior (`(min-width: 80rem) 972px, (min-width: 48rem) 76vw, 170vw`) era una cota superior solo con la lámina entera en alto: en celular apaisado (inView, rejilla md) el marco mide ~390 px a 844×390 y se declaraban 641 (a DPR 3 bajaba una imagen de 2048 px en lugar de la de 1200, ~120 KB de más), y a 1366×657 se declaraban 972 para un marco de 790. Ahora la primera entrada es la fórmula del marco: `min(2 · col, max(col, 1,5 · (100vh − 8rem)))` con `col = 5/12 · min(100vw, 80rem) − 3rem`, redondeada hacia arriba, con `vh` (≥ `svh`, cota superior). En celular, `calc(187.5vw - 5.625rem)` (1,875 × (100vw − 3rem); el 170vw anterior se quedaba corto por encima de ~515 px de ancho). Detrás de cada valor exacto queda el sencillo de antes por si un navegador no entendiera `min()`/`max()`/`calc()` en `sizes` (se salta la entrada). El `srcset` no cambia (next/image deduce los anchos de los `vw` sueltos: 76 y 188). Medido con un srcset denso de prueba: la ranura queda entre 1,003 y 1,008 × el marco en 21 viewports de 320×568 a 2560×1080; lo fija la prueba «tamaño de la imagen de la lámina».
23. **Indicador global de avance (`ScrollProgress`)** (*ronda 2*, fuera del ámbito estricto de T7 pero lo destapa la lámina). Con `right-6` caía justo sobre el borde derecho del contenido `max-w-7xl px-6` en todo ancho ≤ 80rem: sobre el borde de la lámina (1280×800, 1024×768, 844×390) y, a 1024×1366, colgando del contador. Pasa a `right-2`, en el margen (15 px libres hasta la lámina). Además, la pista llevaba `opacity-[0.08]` y el relleno, su hijo, `opacity-60`: la opacidad se multiplicaba (~5 %) y el avance apenas se veía. Ahora el alfa va en el color de fondo (`bg-[#0a0a0a]/8` y `bg-[#0a0a0a]/60`), que era la intención. Monocromo, sin azul. `e2e/motion.spec.ts` (scaleY, relleno del alto de la pista) sigue igual.
24. **`@tabler/icons-react` queda sin importadores** al borrar `FlagshipAI.tsx` (su único uso). No se toca `package.json` aquí (P1 también lo cambia: conflicto seguro). **Pendiente para T11:** quitarlo de `dependencies` y del lockfile (`npx -y npm@11 uninstall @tabler/icons-react --package-lock-only`, o editar `package.json` y `npx -y npm@11 install --package-lock-only`), de `optimizePackageImports` en `next.config.ts` y de `.claude/skills/orbexs-design-system/components.md` (donde figura como librería de iconos de sección). No pesa en el bundle: nada lo importa.
25. **Pruebas añadidas en la ronda 2** (en el spec temporal `e2e/home-t7.spec.ts`): 320×568 en es y en (ninguna palabra de la tesis ni título de capacidad se sale de su caja, sin scroll horizontal); tamaño de la imagen de la lámina a 1440×900, 1366×657, 1024×768, 844×390 y 390×844 (la ranura que calcula el navegador con el `sizes` real cubre el marco y no lo pasa en más de un 5 %); celular apaisado 844×390 (nodos ≥ 9 px, anillo ≥ 18 px, nodos en los extremos de sus aristas, aristas ≥ 1 px); indicador de avance a 1280×800 y 1024×768 (a la derecha de la lámina con margen, dentro del viewport, relleno con alfa ≥ 0,5 y sin opacidad heredada). Contra el código anterior fallan 6 de las 10 (la de en a 320 px ya pasaba).
26. **Integración (F4, sobre P1 `25f4227`).** `page.tsx` conserva los imports estáticos y `HeroMedia` de P1 y añade `Thesis`, `CapabilitiesIndex` y `CapabilitiesMedia` en el orden de §3.5; el spec temporal se fundió al final de `e2e/home.spec.ts` con el prefijo «T7 ·» (24 pruebas; la suite pasa de 147 a 171). JS de `/es` al integrar: **220 233 B** en 14 scripts (+638 B sobre los 219 595 B de P1; tope de T7, +4 000 B). Es la línea base de T8.
...
**Archivos compartidos:** `src/content/dictionaries/es.ts`, `src/content/dictionaries/en.ts`, `src/app/[locale]/page.tsx`, `src/app/globals.css` (bloque «Del papel al dato»: estado final sin JavaScript, decisión 23, y las variantes locales `dossier-compact` y `dossier-narrow`, decisión 8), `src/components/providers/SmoothScroll.tsx` (anclas nativas y URL con ancla, decisiones 28 y 31), `e2e/home.spec.ts` (se añaden bloques al final), `e2e/helpers.ts` (se amplía).
...
### Archivos
- **Crear:** `src/components/sections/home/dossier-progress.ts` (módulo puro), `src/components/sections/home/Dossier.tsx` (sección de servidor: renderiza el expediente) y `src/components/sections/home/DossierStage.tsx` (isla cliente; decisión 10).
- **Modificar:** `src/content/dictionaries/es.ts`, `src/content/dictionaries/en.ts` (clave `dossier`), `src/app/[locale]/page.tsx`, `src/app/globals.css` (bloque «Del papel al dato»: estado final sin JavaScript, decisión 23, y variantes locales de compactación, decisión 8), `e2e/helpers.ts` (`settledTopOffset`), `e2e/home.spec.ts`. *Revisión, ronda 5:* `src/components/providers/SmoothScroll.tsx` (anclas nativas y URL con ancla) y un comentario en `src/components/sections/home/HomeHero.tsx` (decisión 28).
- **Borrar:** nada.
...
**1.2 Importaciones.** Amplía la cabecera de `e2e/home.spec.ts` (sin duplicar ni pegar encima). Al terminar, queda exactamente así (incluye lo que usan los bloques de `e2e/home-t8.spec.ts` tras las revisiones de T8, decisiones 18, 21 y 25: `DOSSIER_FIELD_COUNT`, `DOSSIER_STEP_MS`, `extractionAtStep`, `extractionComplete`, el tipo `FieldState` y `ORBEXS_BLUE`):
```ts
import { test, expect, type Locator, type Page } from '@playwright/test'
import es from '../src/content/dictionaries/es'
import en from '../src/content/dictionaries/en'
...
**1.3 Pega al final de `e2e/home.spec.ts`.** El bloque de abajo es la versión inicial. Al integrar, la fuente es `e2e/home-t8.spec.ts` (decisión 13): desde la marca `// ── Tarea 8` hasta el final, con los helpers locales `expectVisibleStatus`, `openDossierInView`, `fieldsTop`, `fieldsIntoBandY`, `emulateWebKitSvgTargets` y `scrollThroughDossier` y las pruebas de las revisiones (decisiones 18, 21, 25, 27 y 29). La prueba «sin JavaScript» de la versión inicial esperaba «00 / 06»: desde la ronda 3 se pinta el estado final y el contador se oculta (decisión 23).
```ts
// ── Tarea 8 — Del papel al dato ────────────────────────────────────────────
...
Archivo completo: `src/components/sections/home/dossier-progress.ts` (al día con el código tras la ronda 5 de la revisión: el número de campos sale de `DOSSIER_FIELDS`, decisión 16; lo usa la isla `DossierStage.tsx`, decisión 10; y en celular el primer campo se enciende al dispararse la secuencia, decisiones 22 y 26)
```ts
...
> **Borrador superado.** Manda el código de `Dossier.tsx` (sección de servidor, con la imagen) y `DossierStage.tsx` (la isla), que se apartan de este bloque en lo que recogen las decisiones 8, 10, 14, 15, 22, 23 y 26: la imagen llega como slot `media`; la compactación son las variantes locales `dossier-compact:` y `dossier-narrow:` (antes, `[@media(max-height:48rem)]:stage:`), no `short:stage:`; los enlaces llevan anillo de foco blanco; el contador y la columna de estado tienen ancho fijo; la secuencia de celular arranca una sola vez (`once`), por la región de los campos (un `<div>` en %) o por el panel, con el primer campo al instante; el contador va en la capa de instrumento, y hay CSS propio sin JavaScript en `globals.css`. No copies este bloque.
...
Presupuesto de altura del marco fijo (con `overflow-hidden`). Se midió con las clases de arriba; el código compacta con las variantes locales de la decisión 8 (`dossier-compact:` y `dossier-narrow:`; hasta la ronda 5, `[@media(max-height:48rem)]:stage:`) y no con el `short:` del borrador, con las mismas medidas a 1280×720, 1366×657, 1280×600 y 1024×600:
- **1440×900:** la imagen mide 702 px; texto + panel ≈ 580 px; hay 764 px útiles (900 − `pt-24` − `pb-10`). Cabe.
- **1280×720** (viewport por defecto de Playwright y de axe): ya aplica la variante local de la decisión 8, que compacta título, descripción, huecos y filas. Sin esa compactación el contenido medía 730 px con los rellenos y no cabía.
- **Entre 600 y 768 px de alto** (sigue siendo `scrub`, §3.6), sin compactar, el marco `h-svh` con `overflow-hidden` recortaba el contenido centrado: medido a 1280×600, el eyebrow quedaba en y = 31 (debajo del encabezado fijo de 64 px); a 1024×600, en y = −11 y la última fila en 642 (recortada). Compactado, a 1280×600: eyebrow en 93, última fila en 546; a 1366×657: 122 y 574; a 1024×768, pie del panel en 676 (decisión 8).
- **Red de seguridad:** el marco lleva `stage:h-auto stage:min-h-svh`, como la portada. Si aun así el contenido no cabe, el marco crece en lugar de recortar: el `sticky` fija su parte superior y lo que sobra por abajo solo aparece al terminar el recorrido. Medido: a 1024×600 el marco mide 632 px, el eyebrow queda en 80 y lo único que queda bajo el borde es el relleno inferior del panel. En la franja md estrecha (768–896 px de ancho) eso dejaba filas enteras fuera de la vista hasta la ronda 6 de la revisión (decisión 8).
- `pt-24` (96 px) y, compactado, `pt-20` (80 px) dejan libre la cabecera fija de 64 px cuando `#gobierno` queda en `top = 0`.

#### 2.4 `src/app/[locale]/page.tsx`

Antes (`src/app/[locale]/page.tsx`, tras T7):
```tsx
...
Antes (`src/app/[locale]/page.tsx`, tras T7, que pasa la lámina como slot):
```tsx
...
```
`Dossier` no recibe `media`: es la sección de servidor que renderiza el expediente y se lo pasa a su isla (decisión 10).
...
npm run lint                     # 0 errores
npx tsc --noEmit                 # 0 errores
PORT=3080 npx playwright test e2e/home.spec.ts --reporter=line -g "Del papel al dato"   # 44 en verde (10 iniciales + 4 de la ronda 1 + 2 de la ronda 2 + 7 de la ronda 3 + 6 de la ronda 4 + 8 de la ronda 5 + 7 de la ronda 6)
...
grep -rn "2563eb" src/components/sections/home/Dossier*.tsx   # una sola línea: la constante BLUE de DossierStage.tsx (solo la usa el recuadro activo; decisión 10). En home/ hay más: la cumbre (T6) y el nodo activo (T7)
git status --short               # solo los archivos de "Archivos"; si aparece AGENTS.md: git checkout AGENTS.md
```
...
- En inView la secuencia arranca una sola vez, cuando la región de los
  campos (un <div> en %: WebKit no da caja a un <rect> en
  IntersectionObserver) o el panel llegan a la mitad central del viewport,
  y llega siempre al final; el primer campo se enciende al dispararse
  (decisiones 4, 22 y 26).
- El contador «0N / 06» va en la capa de instrumento (#a3a3a3), como el de
  Capacidades (decisión 2).
- Helper settledTopOffset en e2e/helpers.ts para medir el ancla tras el
  scroll suave.
- Hasta 48rem de alto (scrub) el marco h-svh recortaba el contenido
  centrado: dos variantes locales, dossier-compact: y dossier-narrow: (esta,
  en la franja md estrecha de hasta 56rem de ancho), compactan la sección, y
  stage:h-auto stage:min-h-svh deja crecer el marco si aun así no cabe.
  Prueba a 1280×600, 1366×657, 1024×600, 1024×768, 768×600, 853×600,
  800×790 y 768×800 (decisión 8).
- Sin JavaScript se pinta el estado final, como en la portada y en
  Capacidades (decisión 23).
- Las anclas de la portada (#gobierno, #capacidades) siguen siendo <a>
  nativos; SmoothScroll copia el estado del router a la entrada del
  historial que crean, y solo a una entrada con la URL del ancla (Atrás
  desde otra página vuelve a la home), y respeta una URL con ancla al
  cargar (decisiones 28 y 31).
...
2. **Contador `Campos extraídos 0N / 06`:** `data-dossier-counter` envuelve solo la cifra (`0N / 06`, §3.4); la etiqueta es un `InstrumentLabel` aparte. N = campos terminados. *Ronda 4 de la revisión:* la cifra va dentro de otro `InstrumentLabel` (con `shrink-0 whitespace-nowrap`), en `#a3a3a3` como la etiqueta y como el contador de Capacidades (T7). El diseño §4 pone los contadores de avance de las secciones fijas en la capa de instrumento, y el contrato §3.1 le da `#a3a3a3` sobre oscuro. El borrador de 2.3 la pintaba en blanco, y ninguna decisión lo justificaba. Contraste sobre `#141414`: 7,3:1.
...
4. **`inView`:** la secuencia se dispara **una vez** cuando la región de los campos o el panel llegan a la mitad central del viewport (`useInView(…, { once: true, margin: "-25% 0px -25% 0px" })`, con `amount` por defecto, `"some"`), y llega siempre al final aunque después salgan de esa franja (ronda 3, decisión 22; antes se observaba el marco de la imagen y la secuencia se pausaba al salir). En celular la imagen va antes que el panel. El primer campo se enciende al dispararse y la secuencia termina a los 4,2 s (ronda 4, decisión 26; hasta entonces el primero esperaba un paso, 700 ms, y la secuencia terminaba a los 4,9 s). *Ronda 2 de la revisión:* la versión inicial (`amount: 0.5`, el marco al 50 % visible) no arrancaba nunca en celulares apaisados por debajo de 768 px de ancho. Ahí el marco `aspect-[4/5]` a todo el ancho mide 774–865 px y el viewport 360–414 px (667×375, 736×414, 740×360; medido por el revisor: la proporción visible máxima era 0,42–0,49 y el contador se quedaba en «00 / 06»). La franja central no depende del alto del marco. En el celular vertical el comportamiento no cambia: a 390×844 la franja va de 211 a 633 px, así que arranca con el borde superior del marco en 633 px (antes, en 630 px, al 50 % de 428 px). Prueba «celular apaisado (667×375)»: con el centro del marco en el centro del viewport, un recuadro llega a `active` y el contador termina en «06 / 06». Con `amount: 0.5` falla (comprobado).
...
8. **Pantallas bajas en `scrub` (600–720 px de alto) y tabletas.** El marco fijo es `h-svh` con `overflow-hidden` y centra el contenido: si no cabe, lo recorta por arriba y por abajo, y el eyebrow queda debajo del encabezado fijo (medido: a 1280×600, eyebrow en y = 31; a 1024×600, en −11 y última fila en 642). `short:` (≤ 720 px de alto, la variante de T6) compacta la sección en `scrub`: rellenos `pt-20`/`pb-6`, título `text-4xl`, descripción `text-sm`, huecos menores, panel `p-4` y filas `py-1.5`. Como red de seguridad, el marco lleva `stage:h-auto stage:min-h-svh` (igual que la portada): en tabletas de 768–1100 px de ancho con poco alto el marco crece, el `sticky` fija su parte superior y el final del panel aparece al terminar el recorrido. Nada queda bajo el encabezado, pero lo que sobra por abajo queda fuera de la vista durante todo el tramo fijo: a 1024×600, 8 px del relleno inferior del panel; en la franja md estrecha, hasta la ronda 6, filas enteras (ver abajo). La prueba "scrub en pantallas bajas" lo comprueba a 1280×600, 1366×657 y 1024×600. *Ronda 2 de la revisión:* la compactación pasa a una variante local, `[@media(max-height:48rem)]:stage:`, en lugar de `short:stage:`. Lleva las mismas siete clases, ahora hasta 48rem de alto. La `short:` compartida de T6 no se toca: sigue en 45rem. Entre 45 y 48rem de alto, por ejemplo a 1024×768, el marco crecía a 814 px y el pie del panel (relleno y borde inferior) quedaba en y = 774, bajo el borde de la pantalla, durante todo el recorrido. Medido con el cambio: a 1024×768, marco de 768 px, eyebrow en 148 y pie del panel en 676. A 1280×768 y 1366×768, pie en 647. A 1280×720, 1366×657, 1280×600 y 1024×600, las mismas medidas que con `short:`. 1440×900, 1280×800 y 1024×1366 no cambian (h2 de 48 px). En el CSS de producción, la variante arbitraria queda detrás de `lg:text-5xl` y gana sin subir la especificidad. La prueba añade 1024×768 con `panelInView: true` y ahora exige que quepa el panel entero, no solo su última fila. Con `short:` falla (comprobado). *Ronda 6 de la revisión (lente visual, menor): la franja md estrecha.* Entre 768 y ~890 px de ancho, la columna del texto y del panel (`md:col-span-5` con `gap-x-12`) mide 272–325 px: el h2 ocupa 3 líneas, la descripción 4 y los enlaces 2, y se parten «Fecha de nacimiento», «Correo electrónico», «Fecha de solicitud» y «Campos extraídos». Con el marco más alto que la pantalla, durante todo el tramo fijo quedaban bajo el borde entre 1 y 3 filas del panel, también la que se procesa: el recuadro azul se veía en el papel y su «Procesando», no. Entre 769 y 849 px de alto ni siquiera se compactaba. Medido en es, contra `next dev`, en p = 0,5: a 768×600 y 800×600, marco de 761 px y filas 4–6 fuera; a 800×650, filas 5–6; a 768×700 y 800×700, la 6; a 768×800, 800×790 y 834×800, marco de 882–899 px y la fila 6 cortada; a 853×600, marco de 655 px y la fila 6 cortada. En inglés, lo mismo o una fila menos. *Arreglo:* la variante local pasa a `globals.css` con nombre, porque ahora es una lista de media queries: `dossier-compact` = `(max-height: 48rem), (max-width: 56rem) and (max-height: 54rem)`, con las mismas siete clases. Va en bloque, porque la forma corta de `@custom-variant` parte en las comas. Una segunda variante, `dossier-narrow` = `(max-width: 56rem) and (max-height: 54rem)`, compacta más solo en esa franja: h2 en `text-3xl` (2 líneas) y el panel a 10 px, el límite inferior de la capa de instrumento (diseño §4). Las filas llevan además `tracking-[0.1em]`. Así caben en una línea los nombres, y también «Campos extraídos» con su contador. Los umbrales están medidos, no estimados. Sin compactar, el contenido mide 899 px a 768 de ancho, 882 a 800–834, 826 a 853 y 776 desde 880. Con el umbral en 55rem, el inglés seguía cortando la fila 6 a 881–889 × 600: de ahí los 56rem. *Medido tras el arreglo,* en es y en, a 768, 800, 834, 853, 870, 880, 881, 896 y 900 px de ancho por 600, 650, 700, 768, 769, 800, 864 y 865 de alto: ninguna fila queda fuera de la vista ni bajo el encabezado. En la franja, a 600 px de alto, el marco mide 614–637 px y bajo el borde solo quedan hasta 13 px del relleno inferior del panel, como los 8 px de 1024×600. Desde 650 px de alto, el panel cabe entero. Desde 897 px de ancho y en tableta vertical (768×1024, 820×1180) no cambia nada, y tampoco a 1024×600, 1024×768, 1280×600, 1366×657 ni 1440×900. Capturas revisadas a 800×600, 853×600, 800×790 y 896×864 en es, y a 768×600 en en: la fila activa, con «Procesando», y la etiqueta «Ilustración · datos ficticios» quedan a la vista. *Descartado:* compactar aún más (rellenos, interlineado de la descripción) para que a 600 px de alto quepa también el pie del panel: son 13 px de relleno y ninguna fila, el mismo coste que ya se aceptaba a 1024×600. *Nota de entorno:* la caché de Turbopack en disco (`.next/dev/cache/turbopack`) no invalida las utilidades de Tailwind cuando cambia el cuerpo de una `@custom-variant` de `globals.css`, ni en caliente ni al reiniciar `next dev`: seguía sirviendo el CSS con el umbral anterior (comprobado dos veces). Una variante nueva sí se recoge al reiniciar. Tras cambiar el cuerpo de una variante, hay que borrar esa carpeta antes de mirar la página o de correr Playwright. La prueba «scrub en pantallas bajas» pasa a 8 tamaños (decisión 31).
...
10. **Imagen como slot desde el servidor (implementación; presupuesto de JS §5 T8, acción 5).** El código de 2.3 se reparte en dos archivos: `Dossier.tsx` pasa a ser la sección de servidor (sin directiva) que importa `expediente`, renderiza `<Image … placeholder={coverPlaceholder(expediente)} sizes={IMAGE_SIZES}>` (ver decisión 17) con carga diferida y lo pasa como `media` a `DossierStage.tsx`, la isla (`"use client"`) con el resto del código de 2.3, que después cambió (decisiones 14, 15, 22, 23 y 26). La isla ya no importa `next/image` ni la imagen: la miniatura del import estático no viaja en su JS (comprobado en el build: el data URL del expediente solo está en el HTML). `page.tsx` sigue importando `{ Dossier }` de `home/Dossier` (contrato §3.5), como import estático.
11. **`media` dentro de un Fragment.** Suelto junto al `<svg>` dentro de `FocalCover`, el elemento que llega del servidor provocaba en desarrollo el aviso de React «Each child in a list should have a unique key prop» (dueño: Dossier). Envuelto en `<>{media}</>` es hijo único y el aviso desaparece; no cambia el DOM.
12. **`frameClassName` del `ScrollStage`:** se mantiene el de 2.3 (`stage:flex stage:items-center stage:h-auto stage:min-h-svh`). La única sobrescritura es `stage:h-auto` sobre el `h-svh` del marco; las otras tres clases no chocan con `FRAME_CLASS`, así que no hay sobrescrituras nuevas (presupuesto de JS §5 T8).
13. **Ejecución en paralelo (Fase 3).** Sin T7 integrada, `Dossier` se inserta tras `Services` (Capacidades en el estado actual) y antes de `FlagshipAI`, la posición del contrato §3.5; las pruebas van en el spec temporal `e2e/home-t8.spec.ts` (autocontenido; el integrador lo funde en `e2e/home.spec.ts`).
14. **Foco visible en los enlaces (revisión de T8, ronda 1).** El anillo global de `*:focus-visible` es `colors.accent` = `#0a0a0a`, el mismo color que el fondo de la sección: no se veía (WCAG 2.4.7, diseño §11). Los dos enlaces llevan `focus-visible:outline-white`, como el enlace de sector público de la portada. `transition-colors` también anima `outline-color`: la prueba espera con `expect.poll` al color final.
15. **Panel estable en tabletas en `scrub` (768–~870 px de ancho) y en 844×390 (revisión de T8, ronda 1).** Ahí la columna del panel (`md:col-span-5`) mide 272–304 px. El contador se partía en dos líneas («02 /» y «06», contra §3.4) y la etiqueta de estado también («EN / ESPERA»). Como el ancho del estado cambia con el estado, el nombre cambiaba de líneas y el bloque centrado saltaba ~8 px con cada campo terminado (CLS de 0,005 a 0,013, medido por los revisores). Arreglo: contador `shrink-0 whitespace-nowrap`; columna de estado con el ancho del texto más largo, apilando las tres etiquetas de `content.status` en la misma celda de un `grid` (las inactivas, `invisible` y, hasta la ronda 3, también `aria-hidden`: ver decisión 23), sin anchos afinados por idioma; nombre `min-w-0 break-words`. Medido en es y en a 768×1024, 820×1180, 844×390, 1024×768, 1024×1366, 1280×600 y 1440×900: contador en una línea, y altos de fila y `top` del h2 idénticos en p = 0; 0,03; 0,2; 0,38; 0,62; 0,8 y 0,95. *Descartado:* dar 6 columnas al texto y al panel entre md y lg. A 768 px de ancho el marco de la imagen bajaba a 336 px, los recuadros salían del recorte con `--fx: 10%` y, aun moviendo el encuadre, se perdían las etiquetas del formulario de la página izquierda. *Queda, estable:* en es, entre 768 y ~870 px, algunos nombres («Fecha de nacimiento», «Correo electrónico», «Tipo de trámite», «Fecha de solicitud») y la etiqueta «Campos extraídos» ocupan dos líneas (sin partirse harían falta 268 px útiles en el panel, y hay 224–256). Prueba nueva «tableta vertical en scrub» a 768×1024 y 820×1180.
16. **Número de campos con una sola fuente (revisión de T8, ronda 1).** `DOSSIER_FIELD_COUNT = DOSSIER_FIELDS.length` (`dossier-progress.ts` importa `geometry.ts`, que también es puro) y `DossierContent.fields` es una tupla de la misma longitud (`NamePerItem<typeof DOSSIER_FIELDS>`, mapeo homomórfico). Si un diccionario trae más o menos nombres que recuadros, `tsc` falla. La prueba pura lo repite para es y en.
17. **`sizes` según el alto (revisión de T8, ronda 1).** El ancho de la imagen es max(ancho del marco, 1,5 × su alto), y en md el alto es min(78svh, 48rem). `1152px` fijo pedía de más en pantallas bajas. Ahora `IMAGE_SIZES` = `(min-width: 80rem) and (max-height: 37.3rem) 43.75rem, (min-width: 48rem) and (max-height: 61.5rem) 117vh, (min-width: 48rem) 72rem, 170vw`, todo en rem (§3.6). `72rem` sustituye a `1152px`: con la letra del navegador a 20 px el marco llega a 1440 px. La primera condición cubre el escritorio muy bajo, donde manda el ancho del marco (≤ 699 px). Medido en dev (`currentSrc`): 844×390 a DPR 3 pinta 456 px y pide `w=1920` (antes 3840); 1024×600 a DPR 2, `w=1920` (antes 3840); 1440×900 a DPR 2, `w=3840`, y 390×844 a DPR 3, `w=2048` (sin cambios); 1440×500 a DPR 2, `w=1920`. El srcset sigue empezando en 1200w: Next filtra los anchos por el `170vw`. La aproximación aceptada (entre 48 y 80rem, ventanas más de dos veces más anchas que altas) está en el comentario de `IMAGE_SIZES`.
18. **Pruebas añadidas (revisión de T8, ronda 1).** Tabla de `extractionAtStep` y `extractionComplete`. En reducir movimiento, opacidad 1 en los 6 recuadros. En celular, algún recuadro llega a `active` en azul, y `countVisibleBlue`, muestreado cada 100 ms durante la secuencia, nunca pasa de 1. Foco blanco con teclado en los dos enlaces. Tableta vertical (decisión 15). Con el código anterior fallan las pruebas de foco y de tableta (comprobado), y la de tableta falla también si solo se arregla el contador.

19. **Etiqueta de estado visible (revisión de T8, ronda 2).** Cada fila apila sus tres etiquetas (decisión 15), así que su `textContent` las contiene todas y un `toContainText` pasaría siempre. El helper local `expectVisibleStatus(rows, estados)` comprueba, fila a fila, que la etiqueta del estado se ve (`toBeVisible`) y que las otras dos no (`toBeHidden`). Se usa en scrub a 2,5/6,6 (Extraído ×2, Procesando, En espera ×3), en reducir movimiento y al final de las dos pruebas de celular (Extraído ×6). Si la línea que elige la etiqueta visible pinta siempre «Extraído», la prueba de scrub falla (comprobado).
20. **Tableta vertical: h2 medido respecto al marco fijo (revisión de T8, ronda 2).** En p = 0 el `sticky` puede no estar enganchado aún por una fracción de píxel, según dónde caiga `#gobierno` en el documento. Entonces el `top` del h2 respecto al viewport cambia 1 px por redondeo sin que nada salte. Medido a 768×1024 con `#gobierno { margin-top: 0.5px }`: respecto al viewport, 196/195/195; respecto a `[data-stage-frame]`, 195/195/195. La prueba mide ahora `h2.top − frame.top`, y sigue detectando cualquier salto dentro del marco.
21. **Pruebas añadidas (revisión de T8, ronda 2).** «Celular apaisado (667×375)» (decisión 4). 1024×768 en «scrub en pantallas bajas», con el pie del panel (decisión 8). Etiquetas visibles (decisión 19). h2 respecto al marco (decisión 20). En total, 16 pruebas de T8.
22. **`inView`: una vez, hasta el final, y disparado por los campos o el panel (revisión de T8, ronda 3).** Las tres lentes midieron el mismo fallo: con la pausa de la decisión 4, quien baja a leer el panel (en celular va debajo de la imagen) saca el marco de la franja central antes de los 4,9 s y el panel se queda a medias sin fin, con una fila «Procesando» fija (a 390×844, entre «00 / 06» y «04 / 06» según se baje a 1 000 o a 250 px/s; «00 / 06» si se llega al panel de un salto; en celular apaisado < 768 px el marco no coincide nunca en pantalla con el panel). Contradice el diseño §5 («whileInView, una vez») y §3.6. Arreglo, en tres partes: (1) `once: true`, como Capacidades en T7: una vez disparada, la secuencia llega siempre a «06 / 06»; (2) se observa la región de los campos y no el marco: un `<rect>` sin pintar dentro del SVG con el recuadro que envuelve `DOSSIER_FIELDS` (x 448–563, y 265–595, calculado desde `geometry.ts`; *ronda 4:* ahora un `<div>` en % dentro de `FocalCover`, porque WebKit no da caja a un `<rect>` en IntersectionObserver, decisión 26). En celular apaisado el borde superior del marco (774–865 px de alto) entraba en la franja con los campos aún bajo la pantalla, y la extracción entera, con el único azul, ocurría fuera de la vista (hallazgo menor de la lente visual); (3) el panel también dispara (segundo `useInView` con las mismas opciones), para quien llega a él sin pasar por los campos (un salto, buscar en la página). *Descartado:* conservar la pausa y avanzar mientras el marco o el panel estén en la franja. El diseño pide «una vez», y en celular apaisado la franja (187 px a 667×375) cabe entre los campos y el panel (364 px): quien se parara ahí volvería a ver una fila «Procesando» fija. *Medido en dev, con capturas revisadas:* con el borde del marco 10 px dentro de la franja, 1,7 s después no arranca nada a 390×844, 667×375 ni 740×360 (primer campo en y = 733, 472 y 484). Con la región de los campos 10 px dentro, el campo 0 se activa en y = 623, 272 y 260: a la vista y bajo el encabezado. Salto al panel: «01 / 06» a los 1,6 s y «06 / 06» a los 5,6 s en los tres (con el primer campo a los 700 ms; desde la decisión 26, todo un paso antes). Bajada continua a ~450 px/s hasta el panel: llega en «01 / 06»–«02 / 06» y termina en «06 / 06». A 844×390 y 932×430 (inView con la rejilla de md) y a 360×640 también termina en «06 / 06». A 1440×900 en scrub (2,5/6,6: «02 / 06») y con reducir movimiento no cambia nada. Coste: a 390×844 la secuencia arranca unos 110 px de scroll más tarde (cuando llegan los campos, no el borde del marco), con los 6 recuadros a la vista.
23. **Sin JavaScript, el estado final (revisión de T8, ronda 3; detalle de la lente visual).** El HTML del servidor trae el primer render (scrub, avance 0): recuadros transparentes, «00 / 06» y «En espera» ×6, un proceso que parece no haber arrancado. La portada (T6) y Capacidades (T7, desde la ronda 1 de su revisión) pintan sin JavaScript su estado final, el mismo que con reducir movimiento. Por coherencia, T8 también: bloque `@media (scripting: none)` en `globals.css` que da opacidad 1 a los recuadros (el CSS gana al atributo `opacity` que motion pone en el SVG; trazo `#0a0a0a`, sin azul), pinta el nombre en blanco y «Extraído» en cada fila y oculta el contador. Sin scroll que seguir, el contador no dice nada, como el de Capacidades; «Campos extraídos» queda como título de la lista. Para elegir la etiqueta visible, cada etiqueta apilada lleva `data-dossier-status` (§3.4). Las inactivas pierden el `aria-hidden` redundante de la decisión 15: `visibility: hidden` ya las saca del árbol de accesibilidad, y con `aria-hidden` un lector de pantalla no leería la etiqueta «Extraído» que el CSS muestra sin JavaScript. La prueba sin JavaScript de la versión inicial exigía «00 / 06». Ahora el DOM sigue trayendo «00 / 06» (es el primer render, igual que en el cliente), pero una prueba nueva exige que se vea el estado final y que el contador no se vea. La prueba de 1.3 conserva sus demás comprobaciones.
24. **Plan al día (revisión de T8, ronda 3).** El grep del paso 3 busca solo en `Dossier*.tsx`, porque en `home/` también están la cumbre de T6 y el nodo activo de T7. La plantilla del commit del paso 4 describe el disparador y la compactación actuales. §3.6 cita la variante local del expediente en lugar de `short:`. 2.4 parte del `page.tsx` de T7 (lámina como slot), y §2 y §3.4 recogen `globals.css` y `data-dossier-status`.
25. **Pruebas añadidas (revisión de T8, ronda 3).** A 390×844 y a 667×375: bajada continua hasta el panel a ~450 px/s y salto directo al panel. Las dos exigen «06 / 06» y «Extraído» ×6. A 667×375 y 740×360: con el borde del marco 10 px dentro de la franja, 1,7 s después no hay ningún recuadro activo; la lectura es inmediata, sin reintentos, porque un `toHaveCount(0)` esperaría a que la secuencia terminara y pasaría igual. Con la región de los campos dentro de la franja, el primer recuadro se activa a la vista y bajo el encabezado. Sin JavaScript, el estado final (decisión 23). Mutaciones comprobadas: con el `DossierStage.tsx` de la ronda 2 fallan las 7. Con solo `once` sobre el marco (el arreglo mínimo que proponía un revisor) fallan los dos saltos y las dos de celular apaisado. Sin el disparador del panel fallan los dos saltos, y sin el bloque de `globals.css`, la de sin JavaScript. En total, 23 pruebas de T8.
26. **El disparador en WebKit, el primer campo al instante y el contador (revisión de T8, ronda 4).** (a) *La región de los campos es un `<div>`, no un `<rect>` (lente de calidad, mayor).* WebKit no calculaba la caja de los destinos SVG en IntersectionObserver: en `Source/WebCore/page/IntersectionObserver.cpp`, después de `RenderBox`, `RenderInline` y `RenderLineBreak`, venía `// FIXME: Implement for SVG etc.` y una caja vacía. Comprobado en las ramas `safari-7621-branch` a `safari-7624-branch` de WebKit, que incluyen Safari 26; la `safari-7625-branch` ya calcula esas cajas. En Safari 26 y anteriores, y en los navegadores de iOS, que usan WebKit, el `<rect>` de la ronda 3 no se cruzaba nunca: la caja vacía cae en el origen del SVG, fuera del recorte del marco. La extracción esperaba al panel, con los campos ya fuera de la vista (en celular apaisado, entre 300 y 600 px por encima de la pantalla, según midió la lente de calidad), y las pruebas, que solo corren en Chromium, no lo veían. Ahora es un `<div aria-hidden>` sin pintar dentro de `FocalCover`, detrás del `<svg>`, con `left`, `top`, `width` y `height` en % de `IMAGE_WIDTH` e `IMAGE_HEIGHT` (`.focal-cover` es el mismo espacio 3:2 que el viewBox). Los porcentajes se calculan en el módulo desde `DOSSIER_FIELDS`, así que son idénticos en servidor y cliente. Medido: su caja coincide con la unión de los 6 recuadros, con ≤ 0,02 px de diferencia, a 390×844, 667×375, 740×360, 844×390, 932×430, 768×1024 y 1440×900. Con WebKit emulado (decisión 27), la bajada continua da los mismos recuadros vistos que en Chromium. (b) *El primer campo se enciende al dispararse (lentes de calidad y visual, menores).* Antes, el primer paso esperaba al primer tic de `setInterval`, a los 700 ms. Bajando sin parar, en celular apaisado los campos ya estaban bajo el encabezado fijo o fuera de la pantalla, y la extracción entera, con el único azul, ocurría fuera de la vista: el síntoma del menor de la ronda 3 con scroll continuo, porque aquel arreglo solo cubría a quien se detenía. Ahora el efecto da el primer paso con `setTimeout(advance, 0)`, no con un `setState` síncrono; la limpieza lo cancela, también en el doble montaje de StrictMode. Después sigue con `setInterval` cada 700 ms, y la secuencia termina a los 4,2 s. Medido en dev (Chromium), bajando desde que asoma la sección hasta dejar el panel a 80 px del borde y contando los recuadros enteros bajo el encabezado al encenderse y 200 ms después, al terminar el fundido. A 180, 250, 320 y 450 px/s: 667×375, 3, 2, 1 y 1 de 6 (antes, 0/6 a 250 px/s en la prueba nueva, y también a 450 px/s según la lente visual: el campo 0 se encendía en y ≈ 107 y terminaba el fundido bajo el encabezado); 740×360, 3, 2, 1 y 1 de 6 (antes, 0/6 a 250 px/s); 390×844, 6, 4, 3 y 2 de 6. Siempre termina en «06 / 06», y nunca hay más de un azul visible. Del scroll al primer azul pasan 37–60 ms (antes, un paso: 750–765 ms). Capturas revisadas a 390×844, 667×375 y 740×360 durante la bajada: el primer recuadro azul, entero y bajo el encabezado. *Descartado:* un paso más corto en celular. Con el primer campo al instante, a 250 px/s ya se ven dos; un paso más corto dejaría menos tiempo para leer cada fila del panel, y la lente visual no lo pidió. (c) *El contador, en la capa de instrumento* (lente de cumplimiento, detalle): decisión 2. (d) *Plan al día* (lente de cumplimiento, detalle): 2.3 avisa de que el borrador está superado, como el 2.3 de T7, y el presupuesto de altura cita la variante local de la decisión 8 en lugar de `short:`.
27. **Pruebas añadidas (revisión de T8, ronda 4).** (1) «Celular con el IntersectionObserver de WebKit», a 390×844, 667×375 y 740×360. `page.addInitScript` hace que `IntersectionObserver.prototype.observe` ignore los destinos SVG que no son `<svg>`: así se comporta WebKit antes del arreglo, en el que nunca se cruzan. Con la región de los campos 10 px dentro de la franja y el panel todavía debajo, un recuadro llega a `active`, el primero se ve bajo el encabezado y el contador termina en «06 / 06». (2) A 667×375, entre el scroll que mete la región de los campos en la franja y el primer recuadro activo pasa menos de medio paso (350 ms), medido en la página con un `MutationObserver`. (3) A 667×375 y 740×360, bajada continua a ~250 px/s con `requestAnimationFrame` (helper `scrollThroughDossier`): al menos un recuadro se ve entero bajo el encabezado al encenderse y al terminar su fundido. (4) En la prueba de scrub a 1440×900, el contador es `#a3a3a3`. Mutaciones comprobadas: con el `DossierStage.tsx` de la ronda 3 fallan las 7 comprobaciones (0 recuadros activos con WebKit emulado; 765 ms hasta el primer azul; 0/6 recuadros vistos en las dos bajadas; contador blanco). Con el `<rect>` como disparador y lo demás arreglado fallan solo las 3 de WebKit. Sin el primer paso inmediato fallan solo las 3 de tiempo (750 ms; 0/6 en las dos bajadas). En total, 29 pruebas de T8.
28. **Las anclas de la portada y el historial (revisión de T8, ronda 5; lente visual: 1 mayor y 2 menores).** Es código de T5 y T6, pero está en el recorrido que cierra T8 (foco 2 del traspaso: portada → `#gobierno` → enlaces del expediente). (a) *Atrás tras un ancla (mayor y menor).* «Ver casos de uso para sector público» (`#gobierno`, el `<a>` de `HomeHero`) y «Ver Capacidades de Ingeniería» (`#capacidades`, el `m.a` de `Button`) navegan de forma nativa, y la entrada del historial que crea el navegador nace sin estado (`history.state` null). El App Router de Next ignora un `popstate` sin estado (`app-router.js`: `if (!event.state) return`). Tras seguir un enlace de la sección a otra página, Atrás dejaba la URL en `/es#gobierno` (o `/es#capacidades`) con la otra página a la vista, sin límite de tiempo (medido a 1440×900 y 390×844). *Arreglo:* `SmoothScroll` escucha en fase de captura los clics en anclas de la misma página y, en cuanto existe la entrada nueva (`setTimeout`; `hashchange` por si un navegador navega en una tarea posterior), le copia el estado del router de la entrada de la que se sale, como hace Next con los `pushState` del código de la aplicación (`copyNextJsInternalHistoryState`). Si el clic no navega (Ctrl+clic, otro manejador), el estado sigue ahí y no hace nada. Un solo sitio para todas las anclas nativas, también `#journey` y `#status` de RealTy. *Límite no comprobado:* las pruebas solo corren en Chromium, que navega el ancla justo después del clic; en un navegador que la navegue en una tarea posterior lo cubre `hashchange`, salvo en un segundo clic en el mismo ancla (no dispara `hashchange`), donde dependería del orden de las tareas. *Descartado: `<Link>`, la propuesta de la lente.* Arregla Atrás, pero, medido en dev: (1) con teclado, tras Intro el foco se queda en la portada y el Tab siguiente sube a Capacidades; con el `<a>` nativo el navegador lleva el punto de partida del Tab a la sección y el Tab enfoca «Automatización de gobierno»; (2) un segundo clic en el mismo ancla ya no baja, porque Next solo desplaza si el hash cambia (`onlyHashChange` en `segment-cache/navigation.js`): tras volver arriba, `#gobierno` se quedó a 4 700 px; (3) el clic pedía `/es?_rsc=` al servidor antes de desplazar. Un comentario en `HomeHero` explica por qué el enlace sigue siendo `<a>`. (b) *URL con ancla al cargar (menor).* El efecto de montaje de `SmoothScroll` hacía `scrollTo(0, 0)` aunque la URL trajera `#gobierno`: un enlace compartido a `/es#gobierno`, o una recarga tras el ancla, dejaban al visitante en la portada (`scrollY` 0, con `#gobierno` a 4 700 px a 1440×900 y a 4 682 px a 390×844). Ahora, si el hash nombra un elemento de la página, el efecto no mueve la ventana, ni al montar ni al cambiar de ruta (Atrás a `/es#gobierno`). *Medido tras el arreglo (dev):* Atrás vuelve a la home con `#gobierno` a |top| < 0,5 px en los dos tamaños, y con `#capacidades` arriba; abrir `/es#gobierno` o recargar lo dejan arriba; Atrás y Adelante dentro de la home no cambian (tope y `#gobierno`); el ancla no hace ninguna petición `?_rsc=`; `#journey` de RealTy también guarda el estado. *Pendiente para T11 (añadido a su 2.2):* la regla «Scroll y rendimiento 3D» de `CLAUDE.md` dice que `SmoothScroll` «solo restaura el scroll al tope». *Ronda 6:* el estado guardado va ligado a la URL del ancla, y los clics con modificador no guardan nada (decisión 31).
29. **Pruebas añadidas (revisión de T8, ronda 5).** (1) *«Una vez» y la franja central (lente de calidad, menor).* Ninguna prueba fijaba el `once` ni el margen de `IN_VIEW_TRIGGER`: todas terminan con el panel dentro de la franja, que vuelve a disparar, así que seguían en verde con una secuencia que se pausa al salir (el mayor de la ronda 3) o que arranca en cuanto asoman los campos. Nueva, a 390×844 y 667×375: con la región de los campos a la vista pero bajo la franja (su borde superior al 90 % del alto), 1,7 s después no hay ningún recuadro activo; al entrar en la franja, arranca; de vuelta al mismo sitio, con los campos y el panel fuera de la franja (se comprueba), el contador llega a «06 / 06» y no queda ningún recuadro activo. La espera fija con lectura inmediata solo está donde se comprueba que el estado NO cambia, como en la decisión 25; el final de la secuencia se espera con `toHaveText` y plazo (§3.7; ronda 6, decisión 31). Mutaciones: sin `once`, «00 / 06» en los dos tamaños; sin el margen, un recuadro activo con los campos bajo la franja en los dos. (2) *Anclas e historial (decisión 28).* Atrás desde «Automatización de gobierno» a 1440×900 y 390×844, y desde el primer enlace de Capacidades tras «Ver Capacidades de Ingeniería» a 1440×900: el h1 de la home a la vista y la sección a |top| ≤ 2. Abrir `/es#gobierno` y recargar, a 1440×900 y 390×844: |top| ≤ 2. El ancla sigue siendo nativa: con teclado, el Tab que sigue al Intro enfoca «Automatización de gobierno», y un segundo clic vuelve a bajar. Mutaciones: con el `SmoothScroll.tsx` anterior fallan las 5 primeras (el h1 de la página de destino; `#gobierno` a 4 700 y 4 682 px); con `<Link>` en la portada falla la última (el foco). En total, 37 pruebas de T8.
30. **Plan al día (revisión de T8, ronda 5; lentes de cumplimiento y calidad, detalles).** El bloque de 2.2 es copia exacta de `dossier-progress.ts`: `DOSSIER_FIELD_COUNT = DOSSIER_FIELDS.length`, `DossierStage.tsx` como usuaria y el primer campo al dispararse. La decisión 10 ya no dice «sin cambios». §3.5 (fila 4) y §4 (fila 8) citan también `DossierStage.tsx` y `dossier-progress.ts`, para que el inventario de T11 no los tome por huérfanos. §2 añade `SmoothScroll.tsx`, y §3.7 matiza `waitForHydration`.
31. **Ronda 6 de la revisión (un menor visual, un menor y un detalle de calidad, dos detalles de cumplimiento).** (a) *Pantallas bajas en la franja md estrecha (lente visual, menor):* decisión 8. (b) *Un estado huérfano en el historial (lente de calidad, menor).* En `useNativeAnchorHistory`, el estado del router que se guardaba en cada clic en un ancla de la misma página no caducaba ni iba ligado a una URL. Si el clic no navegaba en esta pestaña (Ctrl, Cmd o Mayús+clic, o un manejador que lo cancela), el oyente de `hashchange` lo copiaba en la primera entrada sin estado que apareciera, aunque fuera de otra página, por ejemplo un fragmento escrito a mano en la barra de direcciones. Con Adelante, Next pintaba entonces la home bajo la URL de esa página: reproducido, con el h1 «Construimos soberanía digital.» en `/es/automatizacion-gobierno#detalle`. Era una regresión de la ronda 5, limitada a ese caso. *Arreglo, en dos partes.* (1) El pendiente guarda también la URL del ancla (`link.href`) y solo se gasta en una entrada con esa URL exacta, que es de la misma página. (2) Como hace `<Link>`, se ignoran los clics que abren el enlace en otro sitio: con un botón distinto del principal, con Ctrl, Cmd, Mayús o Alt, con un `target` distinto de `_self` o con `download`. Como antes, desde una entrada sin estado no se guarda nada: copiar `null` con `replaceState` pasaría por el parche de Next, que guardaría `{}`, y Next respondería al `popstate` siguiente con una recarga completa. *Medido en Chromium, con una página mínima:* el primer clic en un ancla crea la entrada sin estado; el segundo clic en el mismo ancla conserva la entrada y su estado (`history.length` y `history.state`, iguales antes y después). El `setTimeout` de SmoothScroll queda para un navegador que siga la especificación y borre el estado en el segundo clic; en Chromium no se puede ejercitar (decisión 28, límite no comprobado). (c) *Renders sin cambios en inView y static (lente de calidad, detalle).* `onProgress` actualizaba `scrub` en todos los modos. En celular y con reducir movimiento, cada umbral de `extractionAt` que se cruzaba al hacer scroll volvía a renderizar la isla sin ningún cambio visible. Ahora sale con `mode !== "scrub"`, como `measure` en Capacidades (T7). Un efecto sobre el modo recalcula desde `progress.get()` al entrar en scrub, con `queueMicrotask` y no con un `setState` síncrono en el efecto. *Medido en dev* con un gancho mínimo de React DevTools, que cuenta los commits en los que cambia el estado de la isla, en una bajada y subida por Capacidades y el expediente con las dos secuencias ya terminadas: a 390×844 (inView), 23 renders antes y 0 después; a 1440×900 con reducir movimiento, 5 y 0; en scrub, 36 y 36, porque ahí sí cambia el estado. Tras pasar de scrub a inView y volver (cambiando el tamaño de la ventana), el contador coincide con el avance actual. (d) *Espera fija para un estado que sí cambia (lente de cumplimiento, detalle).* La prueba «con los campos bajo la franja no arranca, y una vez disparada termina aunque se vuelva arriba» esperaba un plazo fijo y después leía el contador, contra §3.7. Ahora espera «06 / 06» con `toHaveText` y un plazo de 10 s, y después `toHaveCount(0)`. La espera fija se queda solo donde se comprueba que el estado NO cambia, porque ahí un reintento pasaría en falso (decisiones 25 y 29). Con la mutación sin `once`, la prueba sigue fallando en los dos tamaños: el contador se queda en «00 / 06» y se agota el plazo. (e) *La regla de CLAUDE.md que añade T11 (lente de cumplimiento, detalle).* Al texto de T11 2.2 le faltaba la excepción del diseño §4: las ilustraciones llevan «Imagen ilustrativa», o «Ilustración · datos ficticios» si muestran datos, como el expediente. Sin ella, un agente que aplicara CLAUDE.md podría quitar «datos ficticios» de la etiqueta del expediente. Además, «Archivos compartidos» de T8 cita `SmoothScroll.tsx`, y §2, §3.6, 2.3 y la plantilla del commit citan las variantes nuevas de `globals.css`. (f) *Pruebas.* «Scrub en pantallas bajas» pasa de 4 a 8 tamaños: se añaden 768×600, 853×600, 800×790 y 768×800. Cada tamaño declara si cabe el panel entero (`'panel'`) o solo sus 6 filas (`'rows'`: 1024×600, 768×600 y 853×600). A 1024×600 antes no se comprobaba nada del panel; ahora se exigen sus filas. En todos los tamaños, con el último campo en proceso (avance 5,5/6,6), la fila activa dice «Procesando», se ve entera bajo el encabezado y su recuadro es azul. Anclas: tras Ctrl o Cmd+clic en el ancla, y tras un clic que otro manejador cancela, se sigue el enlace del expediente y se escribe `#detalle`. Con Atrás y Adelante, se tiene que seguir viendo la página de la URL. La espera fija es a propósito: se comprueba que la página NO cambia. Se añade también Atrás tras un segundo clic en el mismo ancla. *Mutaciones comprobadas:* con el `SmoothScroll.tsx` de la ronda 5 fallan las dos pruebas de clic sin navegación, con el h1 de la home. Con solo el filtro de modificadores, falla la del manejador. Con solo la URL ligada, pasan las dos. Sin el `setTimeout` pasan todas, también la del segundo clic, porque en Chromium ese clic no borra el estado. Sin `dossier-narrow` fallan 768×600 y 853×600 (última fila en 720 y 614). Con las clases de la ronda 5 fallan los 4 tamaños nuevos (720, 614, 817 y 833,5). En total, 44 pruebas de T8.
32. **Integración (F4, sobre T7 `27d841d`; `redesign/home` en `3644611`).** La rama ya traía `redesign/home` (merge `405c685`, ronda 3), así que el `merge --squash` no tuvo conflictos: `page.tsx` queda en el orden de §3.5 (Capacidades y después `Dossier`, antes de las secciones antiguas), con imports estáticos y el JSON-LD `FAQPage` intacto. `e2e/home-t8.spec.ts` se fundió al final de `e2e/home.spec.ts` con el prefijo «T8 ·»: la cabecera queda como en 1.2; los seis helpers locales de T8 son nuevos (ninguno existía en `home.spec.ts`), y el bloque es idéntico al del spec temporal salvo el prefijo. Son 44 pruebas, y la suite completa sigue en 215 (171 + 44), ahora en 11 archivos. `docs/superpowers/handoff/wip/task8/` (parches, `README.md` y `ciclo-revision.json`) se borró en el mismo commit; sigue en el historial (`git show 3644611:docs/superpowers/handoff/wip/task8/README.md`). lint y `tsc`, 0 errores. e2e completo: 214/215 en la primera pasada (con la caché de Turbopack borrada); falló la intermitente de T6 anotada en las rondas 4 y 6, esta vez con «la ruta cruza «índice /01» (avance 0.3)», y repetida una vez pasó. Build en verde (75 páginas) y `check:modern-js` OK (let=3534, class=167); el expediente no aparece en ningún chunk. JS de `/es` al integrar (`next start`, 1440×900, 3 ejecuciones iguales): **222 150 B** en 15 scripts (14 más el 404 local de `/_vercel/insights`), **+1 917 B** sobre los 220 233 B de T7 (tope de T8, +3 000 B), con un margen de 47 572 B hasta 269 722 B. Es la línea base de T9.

**Estado al cierre (T8, en paralelo, sin P1 ni T7):** JS de `/es` (`next start`, 1440×900, mediana de 3, `measure-js.mjs`): base del worktree (`ca0dd63`, todavía ES5) 250 849 B en 16 scripts → con T8 252 635 B en 16 scripts: **+1 786 B** (tope +3 000 B). e2e completo 156/156 en verde (146 + 10 de T8), lint, `tsc` y build en verde. Medidas del marco en `scrub`: 1440×900 eyebrow y = 181, última fila 750; 1366×657 eyebrow 122, última fila 574; 1024×600 marco 632 px, eyebrow 80.

**Tras la revisión de T8 (ronda 1):** JS de `/es` con el mismo método: 252 744 B en 16 scripts, +109 B sobre la implementación y **+1 895 B** sobre la base (tope +3 000 B). e2e completo 160/160 en verde (146 + 14 de T8), lint, `tsc` y build en verde.

**Tras la revisión de T8 (ronda 2):** JS de `/es` con el mismo método: 252 777 B en 16 scripts, +33 B sobre la ronda 1 (son las clases de la variante local, que van en la isla) y **+1 928 B** sobre la base (tope +3 000 B; margen de 16 945 B hasta 269 722). e2e completo 162/162 en verde (146 + 16 de T8), lint, `tsc` y build en verde. Nota de entorno: el puerto 3080 seguía ocupado por un `next-server` huérfano con cwd en este worktree. El agente no tenía permiso para cerrarlo ni para inspeccionarlo, así que las ejecuciones se hicieron en el puerto 3088 con `CI=1`: así Playwright arranca su propio servidor y nunca reutiliza otro. En la primera pasada de la suite completa, el servidor de desarrollo empezó a devolver «Internal Server Error» en los últimos archivos (seo, smoke, wizard: 42 fallos, todos con la página de error 500). Repetidos una vez, esos tres archivos pasaron 58/58 y la suite completa, 162/162.

**Tras la revisión de T8 (ronda 3), ya sobre P1 y T7:** la rama trae `redesign/home` (P1 `25f4227`, T7 `27d841d` y el traspaso) con un merge, y `page.tsx` queda en el orden del contrato §3.5. JS de `/es` (`next start`, 1440×900, mediana de 3, `measure-js.mjs`; las tres ejecuciones iguales): 221 679 B en 15 scripts (14 más el 404 local de `/_vercel/insights`), **+1 446 B** sobre T7 (220 233 B; tope de T8 +3 000 B), con un margen de 48 043 B hasta 269 722 B. `check:modern-js`: OK (let=3529, class=167). e2e completo 194/194 (171 tras T7, 16 de T8 al traer la rama de integración y 7 de la ronda 3); las 7 nuevas pasan 21/21 con `--repeat-each=3`. Lint, `tsc` y build en verde. El expediente no aparece en ningún chunk de `.next/static/chunks`.

**Tras la revisión de T8 (ronda 4):** JS de `/es` con el mismo método (`next start`, 1440×900, mediana de 3; las tres ejecuciones iguales): 221 764 B en 15 scripts, +85 B sobre la ronda 3 y **+1 531 B** sobre T7 (220 233 B; tope de T8 +3 000 B), con un margen de 47 958 B hasta 269 722 B. `check:modern-js`: OK (let=3529, class=167). El expediente sigue sin aparecer en ningún chunk, y en el `#gobierno` del HTML prerenderizado solo quedan los 6 `<rect>` de los campos. e2e: `e2e/home-t8.spec.ts` 29/29; las 7 comprobaciones nuevas o reforzadas pasan 21/21 con `--repeat-each=3`. Suite completa: 200/200 (194 + 6 de la ronda 4). Lint, `tsc` y build en verde. *Prueba intermitente anotada (de T6, no de T8):* en una pasada anterior de la suite, «portada · scrub en tableta vertical grande › la ruta no cruza el índice…» (`e2e/home.spec.ts`) falló, y repetida una vez volvió a fallar, también con el `DossierStage.tsx` de la ronda 3. Su helper `routePaints` compara dos capturas del recorte, con la ruta y sin ella, y toma cualquier byte distinto como ruta. Medido en este contenedor: dos capturas seguidas de la misma escena, sin cambios, difieren a veces en 3 de 3 672 píxeles, con Δ = 1 en un canal, y la ruta pasa lejos del índice (x ≥ 540 frente a x 24–177, a 1024×1366 y avance 0,7). En la pasada final, 200/200. No se ha tocado.

**Tras la revisión de T8 (ronda 5):** la rama ya traía `redesign/home` (`3644611`, sin commits nuevos desde el merge `405c685`), así que no hizo falta otro merge. JS de `/es` con el mismo método (`next start`, 1440×900, mediana de 3; las tres ejecuciones iguales): 221 992 B en 15 scripts, +228 B sobre la ronda 4 (el arreglo de `SmoothScroll`, que va en el JS común del layout) y **+1 759 B** sobre T7 (220 233 B; tope de T8 +3 000 B), con un margen de 47 730 B hasta 269 722 B. `check:modern-js`: OK (let=3532, class=167). e2e: `e2e/home-t8.spec.ts` 37/37; las 8 nuevas pasan 24/24 con `--repeat-each=3`, y las 6 de anclas e historial también contra `next start`. Suite completa: 208/208 (200 + 8 de la ronda 5). Lint, `tsc` y build en verde.

**Tras la revisión de T8 (ronda 6):** sin commits nuevos en `redesign/home` (sigue en `3644611`), así que no hubo merge. JS de `/es` con el mismo método (`next start`, 1440×900, mediana de 3; las tres ejecuciones iguales): 222 150 B en 15 scripts, +158 B sobre la ronda 5 (el filtro y la URL ligada de `SmoothScroll`, en el JS común del layout, y las clases de las variantes nuevas, en la isla) y **+1 917 B** sobre T7 (220 233 B; tope de T8 +3 000 B), con un margen de 47 572 B hasta 269 722 B. `check:modern-js`: OK (let=3534, class=167). En el CSS de producción, los bloques de `dossier-compact` y `dossier-narrow` van detrás de `lg:text-5xl`, `md:text-[11px]` y `stage:pt-24`, así que ganan sin subir la especificidad. El expediente sigue sin aparecer en ningún chunk. e2e: `e2e/home-t8.spec.ts` 44/44. Suite completa: 214/215 en la primera pasada, con la caché de Turbopack recién borrada. Falló una vez «portada · scrub en tableta vertical grande › la ruta no cruza el índice ni la capa de instrumento…» (`e2e/home.spec.ts`, de T6): «la ruta cruza «Imagen ilustrativa» (avance 0.3)». Es la misma prueba intermitente anotada en la ronda 4, y repetida una vez pasó. Lint, `tsc` y build en verde.
...
```md
- **Home "Cartografía soberana"** (`src/components/sections/home/`): monocromo Orbexs. El azul `#2563eb` significa "lo que el sistema está procesando ahora" y solo lo llevan el punto de la cumbre (portada), el nodo activo (Capacidades) y el recuadro del campo activo (Del papel al dato); nunca hay más de un elemento azul visible a la vez (`e2e/home.spec.ts` lo verifica). Las imágenes viven en `design/source/` y se procesan con `npm run images` (salida en `src/assets/images/` y `public/images/og/`); no se generan imágenes nuevas y las ilustraciones llevan la etiqueta "Imagen ilustrativa" (o "Ilustración · datos ficticios" si muestran datos, como el expediente de Del papel al dato: diseño §4). Las superposiciones SVG usan `viewBox="0 0 1536 1024"` dentro de `FocalCover` y sus coordenadas viven en `home/geometry.ts`. El movimiento solo anima `transform`, `opacity` y `pathLength`; la altura larga y el `sticky` los pone CSS (`stage:`), y `useStageMode()` decide `scrub` / `inView` / `static`.
```
...
En la regla **"Scroll y rendimiento 3D"**, `SmoothScroll` ya no solo vuelve al tope (T8, decisión 28). Sustituye:
```md
`SmoothScroll` (`src/components/providers/SmoothScroll.tsx`) solo restaura el scroll al tope en cada navegación; el desplazamiento suave a anclas lo da `scroll-behavior: smooth` nativo (`src/app/globals.css`).
```
por:
```md
`SmoothScroll` (`src/components/providers/SmoothScroll.tsx`) restaura el scroll al tope en cada navegación, salvo si la URL trae un ancla que existe en la página (`/es#gobierno`); el desplazamiento suave a anclas lo da `scroll-behavior: smooth` nativo (`src/app/globals.css`). Los enlaces a anclas de la misma página son `<a>` nativos, no `<Link>` (foco del teclado y segundo clic), y `SmoothScroll` copia el estado del router de Next a la entrada del historial que crean, para que Atrás funcione.
```
