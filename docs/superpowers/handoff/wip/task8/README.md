...
| 3 | **cambios: el mismo mayor** (confirmado de forma independiente; mismo arreglo propuesto) + 1 detalle: en el plan, el grep del paso 3, la plantilla del commit del paso 4 y §3.6 aún describen la versión inicial (marco al 50 %, `short:`) | **cambios: 1 mayor.** En celular (`inView`) la secuencia depende solo de `useInView(frameRef)`: al bajar hasta el panel (debajo de la imagen) el marco sale de la franja central y la secuencia se congela en «00/06» o «01/06» con una fila «Procesando» fija; si se llega al panel de un salto, ni arranca. Arreglo propuesto: `ref` en el panel, segundo `useInView(panelRef, { margin: "-25% 0px -25% 0px" })` y `running = frameInView \|\| panelInView` (o semántica «una vez» con `once: true`). + 1 detalle: el grep del paso 3 del plan anuncia «una sola línea» y también encuentra `HeroRoute.tsx`. | **cambios: el mismo mayor** (arreglo mínimo que propone: `once: true` en el `useInView` del marco, para que la secuencia, una vez disparada, llegue siempre al final, como Capacidades en T7) + 1 menor: en celular apaisado < 768 px la secuencia arranca con los campos aún fuera de pantalla (propone observar la región de los campos con un `<rect>` invisible dentro del SVG) + 1 detalle: sin JS muestra «00 / 06» (la prueba del plan lo exige; decidir si pasa al estado final, como T7) | **pendiente (ronda 4, en el Mac)** |
...
## Qué debe hacer el corrector de la ronda 4
1. **Mayor (los tres revisores):** en `inView` la secuencia se congela si el marco sale de la franja central antes de terminar. Que, una vez disparada, llegue al final (p. ej. `once: true`), o que avance mientras el marco **o** el panel estén en la franja. Añadir una prueba que baje hasta el panel a ritmo normal en 390×844 y en 667×375 y exija «06 / 06».
2. **Menor (visual):** en celular apaisado < 768 px, disparar cuando la región de los campos (no el borde del marco) entra en pantalla.
3. **Detalles:** el grep del paso 3, la plantilla del commit del paso 4 y §3.6 del plan describen la versión inicial; decidir y documentar el estado sin JS (la prueba actual exige «00 / 06»; T7 muestra el estado final sin JS).
Después: una ronda 5 de revisión con las tres lentes y la integración con `home-integrate-serial`.
