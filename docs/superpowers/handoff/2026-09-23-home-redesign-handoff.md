...
| **T8** del papel al dato | — (sin integrar) | Implementada (`5248386`) y corregida en 2 rondas (`57042ff`, `e445319`): foco visible sobre `#0a0a0a`, panel estable en tabletas, arranque en celular apaisado. **Guardada como parches** en `docs/superpowers/handoff/wip/task8/` (base `ca0dd63`). Ronda 3 completa: los tres revisores piden el mismo arreglo mayor (en celular la secuencia se congela al bajar al panel); su corrector queda para el Mac. |
...
**Trabajo sin integrar:** `docs/superpowers/handoff/wip/task8/` (parches `git format-patch`, `README.md` con el estado de cada ronda y cómo retomarla, `ciclo-revision.json` con las decisiones y los hallazgos). La ronda 3 de T8 terminó (tres lentes, un mismo mayor); el README lista lo que debe hacer el corrector de la ronda 4.

**Lo que falta ahora, en orden** (sustituye a §8.3 donde difiera):
1. **T8:** aplicar los parches (`README.md` de `wip/task8`), corregir los hallazgos de la ronda 3 (lista en el README), una ronda más de revisión con `home-task-cycle` (`impl` = resultado del implementador) y dejarla lista para integrar.
