...
| **T8** del papel al dato | «feat(home): del papel al dato, extracción de 6 campos sobre el expediente» (el commit que actualiza esta fila; un commit no puede citar su propio hash) | e2e **215/215** (en la primera pasada, 214: falló la intermitente de T6 «la ruta no cruza el índice…», que repetida una vez pasó), build ✅, `check:modern-js` OK. JS de `/es` **222 150 B (+1 917)**. 6 rondas de revisión con tres lentes (1: foco invisible sobre `#0a0a0a` y panel roto en tabletas; 2: celular apaisado sin arrancar; 3: la secuencia se congelaba al bajar al panel; 4: disparador SVG sin caja en WebKit y primer campo tarde; 5: Atrás tras un ancla de la portada; 6: sin mayores, franja md estrecha e historial ligado a la URL). Integrada sin conflictos: su spec temporal se fundió en `e2e/home.spec.ts` con el prefijo «T8 ·», y `wip/task8/` se borró. Decisiones en el commit y en la sección de T8 del plan (1–32). |
...
**Trabajo sin integrar:** ya no queda nada de T8. Las rondas 3 a 6 se corrigieron y T8 se integró (fila de arriba); `docs/superpowers/handoff/wip/task8/` (parches, `README.md` y `ciclo-revision.json`) se borró en ese mismo commit y sigue en el historial (`git show 3644611:docs/superpowers/handoff/wip/task8/README.md`).

**Lo que falta ahora, en orden** (sustituye a §8.3 donde difiera):
1. **T8:** hecho. Rondas 3 a 6 corregidas e integrada (fila de T8 de la tabla).
...
3. **Integración en serie** T8 → T9 → T10a con `home-integrate-serial` (`measure: true`), y **T10b** con `home-task-cycle`. T8 ya está integrada: para T9, `previousJs: "222 150 B (T8)"`.
