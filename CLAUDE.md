...
- **NO DESTRUYAS EL SEO:** El archivo `src/app/[locale]/page.tsx` contiene un schema dinámico `@type: "FAQPage"` generado desde `dict.faq.items`. Si modificas la sección de FAQ (diccionarios `es.ts`/`en.ts`), el schema se sincroniza solo — no lo elimines. Cada página define su `canonical`/`hreflang` con `buildAlternates()` de `src/lib/i18n.ts`.
